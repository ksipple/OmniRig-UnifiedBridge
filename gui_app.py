...
class BridgeGUIApp(tk.Tk):
    ...
    def open_omnirig_dialog(self):
        ...
        def run():
            ...
            if exe_path:
                ...
                config.ui_print("❌ OmniRig executable not found on this system. Please check your installation.")
    ...
    def force_kill_omni_process(self):
        if messagebox.askyesno("Confirm Process Kill", "Are you sure you want to force terminate OmniRig.exe via taskkill?"):
            config.ui_print("⚠️ Initiating emergency process termination sequence...")
            try:
                if os.system("taskkill /f /im OmniRig.exe") == 0:
                    config.ui_print("✅ Process OmniRig.exe was terminated successfully.")
                else:
                    config.ui_print("⚠️ Request dispatched (Process may already be inactive).")
    ...
    def update_gui_indicators(self):
        ...
        self.lbl_omni.config(text="OmniRig: Connected" if config.status_states["omnirig"] == "online" else "OmniRig: Offline / Disconnected", fg="#ffffff" if config.status_states["omnirig"] == "online" else "#ff8888")
        ...
        self.lbl_wave.config(text="Wavelog Cloud: Connected" if config.status_states["wavelog"] == "online" else "Wavelog Cloud: Unreachable", fg="#ffffff" if config.status_states["wavelog"] == "online" else "#ff8888")
